...
export default class ContractorsScene extends Phaser.Scene {

    // private control?: Control;
    private scrollbar?: Phaser.GameObjects.Graphics;
    private scrollView?: Phaser.GameObjects. Container;
    private contractors: string[];
    private contractorsWindow: number[];
    ...
    create() {
        ...
        // add card
        const card: Phaser.GameObjects.Image = this.add.image(INIT_MAIN_UI_X * 4.8, INIT_MAIN_UI_Y * 11.5, 'card').setScale(2.5, 2);

        // add arrows
        const ARROW_DIST: number = 500;
        const leftArrow: Phaser.GameObjects.Image = this.add.image(card.x - ARROW_DIST, 150, 'left-arrow');
        const rightArrow: Phaser.GameObjects.Image = this.add.image(card.x + ARROW_DIST, 150, 'right-arrow');
        ...
        let leftHeading: CustomButton = new CustomButton(this, middleHeading.x - middleHeading.width - HEADING_DIST, 150,'button1Normal', 'button1Hover', 'Plumbers', 30).setScale(0.75, 0.75);
        ...
        // add subheadings
        this.add.text(this.scrollView.x, this.scrollView.y - SUBHEADING_SPACE_Y, `\t\t\t\tCompany Name\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\tRating\t\t\t\t\t\t\tRate/Day`, {
            fontSize: 40,
            color: '#ffffff'
        });
        ...
        let names:Phaser.GameObjects.Text[] = [];
    ...
    // @ts-ignore
    generateContent(INIT_MAIN_UI_X: number, INIT_MAIN_UI_Y: number, CONTENT_BUFFER_X: number, SUBHEADING_SPACE_Y: number, names: Phaser.GameObjects.Text[], ratings: Phaser.GameObjects.Image[], prices: Phaser.GameObjects.Text[], clickAreas: Phaser.GameObjects.Graphics[], infoNum: number): void {
        // put random info[default = 20 messages]
        for (let i = 0; i < infoNum; i++) {
            // add each contract
            // @ts-ignore
            const scrollViewContent:Phaser.GameObjects.Container = this.add.container(this.scrollView.x / 10, this.scrollView.y / 6 * i);
            // @ts-ignore
            this.scrollView.add(scrollViewContent);

            // name
            const generateName: string = generateRandomCompanyName();
            const companyName: Phaser.GameObjects.Text  = this.add.text(scrollViewContent.x + CONTENT_BUFFER_X, scrollViewContent.y, generateName, {
                fontSize: 40,
                color: '#ffffff'
            });
            ...
            // rating
            const generateRating: string  = generateRandomRating();
            const ratingImage:Phaser.GameObjects.Image = this.add.image(scrollViewContent.x + 1000, scrollViewContent.y + 18,  generateRating)
                .setScale(0.1, 0.1)
            ...
            // price
            // @ts-ignore
            const generatePrice: number = generateRandomPrice(ratingImage.texture.key);
            const price: Phaser.GameObjects.Text = this.add.text(scrollViewContent.x + 1200, scrollViewContent.y+3, `$${generatePrice}`, {
                fontSize: 40,
                color: '#ffffff'
            });
            ...
            const areaCheck1: Phaser.Geom.Rectangle = new Phaser.Geom.Rectangle(scrollViewContent.x, scrollViewContent.y - 10, 1380, 60)
            const areaCheck2: Phaser.Geom.Rectangle = new Phaser.Geom.Rectangle(INIT_MAIN_UI_X * 1.05 + 40, INIT_MAIN_UI_Y * 6 - 10, 1382, 571)

            clickArea.setInteractive(areaCheck1, Phaser.Geom.Rectangle.Contains)
                .on(Phaser.Input.Events.GAMEOBJECT_POINTER_DOWN, function (pointer: any): void {
    ...
    // @ts-ignore
    createWindow (func): void
    {

        var handle: string = 'window' + this.count++;
        var win: Phaser.GameObjects.Zone = this.add.zone(400, 400, func.WIDTH, func.HEIGHT).setInteractive().setOrigin(0);
        ...
        // @ts-ignore
        win.on('drag', function (pointer: Phaser.Input.Pointer, dragX: number, dragY: number): void {
            // @ts-ignore
            this.x = dragX;
            // @ts-ignore
            this.y = dragY;
    ...
    shiftHeading(leftHeading: CustomButton, middleHeading: CustomButton, rightHeading: CustomButton, direction: string, cardxPos: number, HEADING_DIST: number): void {
        let left: number = this.contractorsWindow[0];
        let middle: number = this.contractorsWindow[1];
        let right : number = this.contractorsWindow[2];
    ...
    destroyWindowContents(names: Phaser.GameObjects.Text[], ratings: Phaser.GameObjects.Image[], prices: Phaser.GameObjects.Text[], clickAreas: Phaser.GameObjects.Graphics[]): void {
...
function generateRandomCompanyName() {
    ...
    const companyNameLength: number = Phaser.Math.Between(5, 10);
    const suffixesNameNum: number = Phaser.Math.Between(0, 10);

    let companyName: string = '';
    for (let i: number = 0; i < companyNameLength; i++) {
        if (i % 2 === 0) {
...
function generateRandomPrice(ratingImageName: string): number {
    ...
    const star: number = parseInt(ratingImageName.charAt(0))
